...
export default async function (page, out) {
  await page.evaluate(() => {
    ...
    g.autopilot = (dt) => {
      ...
      if (goal) {
        ...
        if (!tgt || bd >= 45) { p.camYaw = want; p.camPitch = -0.1; inp.keys.add('w'); }
        else {
  ...
  await run(40, 'fight', { godMode: true, tpAt: 60 });
  await snap('fight');
  await run(40, 'loot', { godMode: true, tpAt: 60 });
  await snap('loot');
  await run(40, 'tp', { godMode: true, tpAt: 0 });
  await snap('tp');
  for (let i = 0; i < 6; i++) {
    const r = await run(20, 'event' + i, { godMode: true, tpAt: 0 });
    if (r.state === 'transition') break;
    if (i === 1) await snap('boss');
  }
